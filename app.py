...
# Deployment version：V11 short selling + indicator stable + cloud-ready
...
st.caption("V11：新增放空 / 回補功能；支援多單、空單、全部平倉，並保留 V10 指標穩定修正。")
st.markdown("---")
...
def record_trade(row: pd.Series, side: str, shares: int, price: float, reason: str) -> None:
    # total_equity = 現金 + 持倉市值；空單 shares 為負數，所以會自動扣掉回補負債。
...
def short_shares(row: pd.Series, lot_count: int, reason: str) -> tuple[bool, str]:
    ...
    current_short = abs(int(st.session_state.shares))
    ...
    return True, f"成功放空 {lot_count} 張，成交價 {price:.2f}"
...
def cover_shares(row: pd.Series, lot_count: int, reason: str) -> tuple[bool, str]:
    ...
    st.session_state.cash -= cover_cost
...
with st.sidebar:
    st.header("⚙️ 闖關設定")
    st.caption("目前版本：V11-short-selling（新增放空 / 回補，並保留按下一根不清掉指標）")
...
with t3:
    if st.button("🔴 買入做多", type="primary", use_container_width=True):
...
with t4:
    if st.button("🟢 賣出多單", use_container_width=True):
...
with t5:
    if st.button("🔵 放空", use_container_width=True):
...
with t6:
    if st.button("🟣 回補空單", use_container_width=True):
